...
## Sample Mean
- **Definition**: The average of a sample, denoted as $\bar{x}$.

## IID Samples
- **Definition**: Samples that are both independent and identically distributed.

## Standard Error
- **Definition**: The standard deviation of the sample mean's sampling distribution, denoted as $SE$.
- **Formula**: $SE = \frac{\sigma}{\sqrt{n}}$, where $\sigma$ is the population standard deviation and $n$ is the sample size.

## Distribution of the Sample Mean
- **Overview**: As $n$ increases, the sample mean's probability density approaches the normal distribution ($N$), regardless of the population's distribution.

## Test Statistic
- **Definition**: A standardized value used for hypothesis testing.
- **Formula**: Often $\frac{\text{observed value} - \text{expected value}}{SE}$.

## T-Distribution
- **Overview**: Used instead of the normal distribution when the sample size is small and the population standard deviation is unknown.
- **Condition**: Sample size is less than 30. It is more spread out than the normal distribution and has heavier tails.

## Quantiles/Percentiles
- **Definition**: Values that divide a sample into equal-sized, ordered groups.
- **Usage**: For determining values below which a certain percentage of the data falls.
